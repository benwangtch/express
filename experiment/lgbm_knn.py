...
parser.add_argument("--selectNum", type=int)
args  = parser.parse_args()
mode = args.type
selectNum = args.selectNum
...
ReportName = f"sigir_knn_{selectNum}_impute_{mode}report.csv"
initReport(f'{ReportPath}/{ReportName}')

def process_row(args):
    idx, row, mode = args
    tmp_dict = row.to_dict()
    groupData = selectByKNN(selectNum, tmp_dict, mode, train_val_data)
...
imputed_test_data.to_csv(f'./data/demo_paper_data/sigir_knn_{selectNum}_impute_{mode}_testFeat.csv', index=False)
